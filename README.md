...
```bash
# From project root
docker build -t clothing_brand_ctr_env-env:latest -f server/Dockerfile .
```
...
### Action
**ClothingBrandCtrAction**: Contains a single field
- `message` (str) - The message to echo back

### Observation
**ClothingBrandCtrObservation**: Contains the echo response and metadata
...
If you already have a Clothing Brand Ctr Env environment server running, you can connect directly:

```python
from clothing_brand_ctr_env import ClothingBrandCtrEnv

# Connect to existing server
clothing_brand_ctr_envenv = ClothingBrandCtrEnv(base_url="<ENV_HTTP_URL_HERE>")

# Use as normal
result = clothing_brand_ctr_envenv.reset()
result = clothing_brand_ctr_envenv.step(ClothingBrandCtrAction(message="Hello!"))
```

Note: When connecting to an existing server, `clothing_brand_ctr_envenv.close()` will NOT stop the server.
...
with ClothingBrandCtrEnv(base_url="http://localhost:8000") as env:
    result = env.reset()
    print(f"Reset: {result.observation.echoed_message}")
    # Multiple steps with low latency
    for msg in ["Hello", "World", "!"]:
        result = env.step(ClothingBrandCtrAction(message=msg))
        print(f"Echoed: {result.observation.echoed_message}")
```
...
app = create_app(
    ClothingBrandCtrEnvironment,  # Pass class, not instance
...
def run_episode(client_id: int):
    with ClothingBrandCtrEnv(base_url="http://localhost:8000") as env:
        result = env.reset()
        for i in range(10):
            result = env.step(ClothingBrandCtrAction(message=f"Client {client_id}, step {i}"))
...
python3 server/clothing_brand_ctr_env_environment.py
```
...
├── client.py              # ClothingBrandCtrEnv client
├── models.py              # Action and Observation models
└── server/
    ├── __init__.py        # Server module exports
    ├── clothing_brand_ctr_env_environment.py  # Core environment logic
