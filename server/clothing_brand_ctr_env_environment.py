...
class ClothingBrandCtrEnvironment(Environment):
    ...
    Example:
        ...
        >>> obs = env.reset()
        >>> print(obs.echoed_message)  # "Clothing Brand Ctr Env environment ready!"
        >>>
        >>> obs = env.step(ClothingBrandCtrAction(message="Hello"))
    ...
    def reset(self) -> ClothingBrandCtrObservation:
        ...
        Returns:
            ClothingBrandCtrObservation with a ready message
        """
        self._state = State(episode_id=str(uuid4()), step_count=0)
        self._reset_count += 1

        return ClothingBrandCtrObservation(
            echoed_message="Clothing Brand Ctr Env environment ready!",
            message_length=0,
            done=False,
            reward=0.0,
        )
    ...
    def step(self, action: ClothingBrandCtrAction) -> ClothingBrandCtrObservation:  # type: ignore[override]
        ...
        Args:
            action: ClothingBrandCtrAction containing the message to echo

        Returns:
            ClothingBrandCtrObservation with the echoed message and its length
        """
